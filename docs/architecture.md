...
This is a deliberate trade. A state file would let capsule remember slightly more
— but it would also be one more thing that can disagree with reality. Kill a
...
skipped, fail, or be missed because a process was killed — the runtime removes
the container and its anonymous volumes when the process ends, whatever ended it.
...
`RunArgs` in particular — the function that turns a `capsule.toml` into a
runtime argv — is where this tool's entire security surface lives: what gets
...
- `packages` installs with whichever of `apk`/`apt-get`/`dnf` the image has. If
  it recognises none, it writes a warning to stderr naming the packages it
...
- `capsule up` prints what will survive before it starts, every time. The promise
  is the product; it gets stated rather than assumed.
